...
(function() {
  $(document).on('ready', function() {
    var element, got_location;
    $(window).on('scroll', function() {
      var scroll_top;
      scroll_top = $(window).scrollTop();
      if (!(scroll_top > $('.nav-bar').height() || $('.nav-bar').hasClass('fixed'))) {
        return $('.nav-bar').removeClass('scrolled');
      } else {
        return $('.nav-bar').addClass('scrolled');
      }
    });
    $('.menu-button').on('click', function() {
      var r;
      r = '.nav-bar';
      if (!$(r).hasClass('exploded')) {
        return $(r).addClass('exploded');
      } else {
        return $(r).removeClass('exploded');
      }
    });
    ...
    $("#contact-form button").on('click', function() {
      return $(this).html('Enviando...');
    });
    $("#contact-form").on("ajax:success", function(e, data, status, xhr) {
      $(this).html(data);
      return $(this).find('button').html('Intentar de nuevo');
    }).on("ajax:error", function(e, xhr, status, error) {
      $(this).find('button').html('Intentar de nuevo');
      return alert("Hubo un error al enviar el correo, intente de nuevo más tarde.");
    });
    got_location = function(position) {
      return $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
          ...
          return $('.distribuidores-listado').html(data);
        },
        error: function(jqXHR, textStatus, errorThrown) {
          return $('.todos-los-distribuidores .titulo').html('No pudimos localizarte... Esta es la lista entera de distribuidores.');
        }
      });
    };
