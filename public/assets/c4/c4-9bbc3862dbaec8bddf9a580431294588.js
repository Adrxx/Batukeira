...
(function() {
  $(document).on('ready', function() {
    var dismissAllNotifications, dismissNotification, j, len, t, textareas;
    $('.c4-menu-button').on('click', function() {
      var isDisplayed;
      isDisplayed = $('.c4-nav').hasClass("nav-displayed");
      if (isDisplayed) {
        return $('.c4-nav').removeClass("nav-displayed");
      } else {
        return $('.c4-nav').addClass("nav-displayed");
      }
    });
    textareas = document.getElementsByTagName('textarea');
    for (j = 0, len = textareas.length; j < len; j++) {
      t = textareas[j];
      t.style.height = "1px";
    ...
    dismissNotification = function(notification) {
      ...
      return $(notification).nextAll('.c4-flash').each(function(i) {
        m = $(this).css("margin-bottom").replace(/[^-\d\.]/g, '');
        return $(this).css("margin-bottom", (m - 50) + "px");
      });
    };
    $('.c4-flash').on("touchend click", function() {
      return dismissNotification(this);
    });
    dismissAllNotifications = function() {
      return $('.c4-flash').each(function() {
        return dismissNotification(this);
      });
    };
    ...
    return $(window).on('scroll', function() {
      ...
      if (!(scroll_top > $('.c4-header').height())) {
        return $('.c4-nav').removeClass('c4-nav-scrolling');
      } else {
        return $('.c4-nav').addClass('c4-nav-scrolling');
      }
    });
  });
...
(function() {
  $(document).on('ready', function() {
    ...
    orderIndexes = function() {
      return $('.mnt-photo-manager').children('.mnt-pm-container').each(function() {
        return $(this).children('.mnt-pm-p-order').val($(this).index());
      });
    };
    $('.mnt-pm-banner').on("dragenter", function() {
      $(this).find('.mnt-pm-normal').hide();
      return $(this).find('.mnt-pm-drop').show();
    });
    $('.mnt-pm-banner').on("dragleave drop", function() {
      $(this).find('.mnt-pm-drop').hide();
      return $(this).find('.mnt-pm-normal').show();
    });
    $('#batch-input').on("change", function() {
      var batch_index, clone, extension, f, file_index, input_field, j, k, len, len1, reader, ref, ref1, valid;
      input_field = this;
      valid = true;
      ref = this.files;
      ...
      if (this.files.length > 0 && valid) {
        ...
        for (k = 0, len1 = ref1.length; k < len1; k++) {
          ...
          reader.onload = function(e) {
            $('.mnt-photo-manager .mnt-pm-spacer').first().before("<div class='mnt-pm-container mnt-pm-normal'> <div class='mnt-pm-photo' style='background-image: url(" + e.target.result + ")'></div> <input type='hidden'name='batches[" + batch_index + "][ordering][" + file_index + "]' class='mnt-pm-p-order'> <input type='hidden'name='batches[" + batch_index + "][year][" + file_index + "]' class='mnt-pm-p-year'> <input type='hidden'name='batches[" + batch_index + "][month][" + file_index + "]' class='mnt-pm-p-month'> <input type='hidden'name='batches[" + batch_index + "][weight][" + file_index + "]' class='mnt-pm-p-weigth'> <input type='hidden' name='batches[" + batch_index + "][deleting][" + file_index + "]' class='mnt-pm-p-destroyer' value='0'> <div class='mnt-pm-delete b'><span class='icon-delete'></span></div> <div class='mnt-pm-comment b'><span class='icon-message'></span></div> </div>");
            file_index++;
            return orderIndexes();
          };
        ...
        $(input_field).hide();
        return $(clone).insertBefore(input_field);
      }
    });
    $('.mnt-photo-manager').on("click", '.mnt-pm-comment', function() {
      var month, photo, weight, year;
      console.log('hola');
      $('.mnt-photo-manager').children('.mnt-pm-normal').each(function() {
        return $(this).find('.mnt-pm-photo').removeClass("mnt-pm-top");
      });
      ...
      return window.currentPhotoContainer = this.parentNode;
    });
    $('.mnt-pm-save, .mnt-full-overlay').on("click", function() {
      var current, month, weight, year;
      ...
      return $('.mnt-full-overlay').fadeOut();
    });
    $('.mnt-photo-manager').on("click", '.mnt-pm-delete', function() {
      $(this).parent().fadeOut();
      return $(this).siblings('.mnt-pm-p-destroyer').val(1);
    });
    $('.mnt-photo-manager').on("mousedown touchstart", ".mnt-pm-photo", function() {
      var current, originX, originY;
      event.preventDefault();
      current = this;
    ...
    return $('.mnt-pm-numeric-input').keydown(function(e) {
